<template>
    <div class="container">
        ...
        <div class="table-container">
            ...
            <el-scrollbar v-else>
                <el-table :data="cron_clocked.data" border table-layout="auto" style="width: 100%"
        ...
        <div class="page-container">
            <el-pagination v-model:current-page="cron_clocked.page" v-model:page-size="cron_clocked.page_size"
                :page-sizes="[20, 40, 50, 100]" layout="total, sizes, prev, pager, next, jumper"
                :total="cron_clocked.total" @size-change="handleSizeChange" @current-change="handleCurrentChange" />
        </div>
    </div>
    <!-- 添加定时任务 -->
...
const { cron_clocked } = storeToRefs(stores);

const isData = computed(() => {
    return cron_clocked.value.data.length > 0 ? true : false;
})
...
async function confirmClick(formEl) {
    ...
    await formEl.validate((valid, fields) => {
        if (valid) {
            stores.addClocked(ruleForm).then((res) => {
                // console.log(res);
                if (res) {
                    stores.getClocked();
...
const handleDelete = (row) => {
    stores.deleteClocked(row.id).then(() => {
...
const editClocked = async (formEl) => {
    if (!formEl) return
    await formEl.validate((valid, fields) => {
        if (valid) {
            stores.updateClocked(editForm.id, editForm).then(() => {
                stores.getClocked();
...
const handleSizeChange = (val) => {
    cron_clocked.page_size = val
    stores.getClocked();
    // loading.close()
}
const handleCurrentChange = (val) => {
    cron_clocked.page = val
    stores.getClocked();
    // loading.close()
}
// 控制表格字段
